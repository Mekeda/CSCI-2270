...
namespace main_savitch_5
{
    ...
    polynomial polynomial::derivative() const
    {
		...
		do
		{
			p_prime.add_to_coef(expo * coefficient(expo), expo - 1);
			expo = next_term(expo);
		}
		while (expo != 0);
    ...
    {		
		polynomial p;
        unsigned int exponent1 = 0;
        unsigned int exponent2 = 0;

        do
        {
			do
			{
                p.add_to_coef(p1.coefficient(exponent1)*p2.coefficient(exponent2), exponent1+exponent2);
                exponent1 = p1.next_term(exponent1);
			}
            while (exponent1 != 0);
            
            exponent1 = 0;
            exponent2 = p2.next_term(exponent2);
        }
        while (exponent2 != 0);
	...
	double epsilon) const
    {
        ...
        {
            double helix = eval(guess);
            if (fabs(helix) < epsilon)
            {
                success = true;
                answer = guess;
            }

            double helixfossil = derivative().eval(guess);
            if (fabs(helixfossil) < epsilon)
                break;
            guess -= helix / helixfossil;
        }
	}
}
