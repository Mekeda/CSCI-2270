...
int main()
	{
		...
		// Converts the array bag into a vector
		if (v.size() == unsigned (aBag.getCurrentSize()))
			cout << "Size of the vector is the same as the size of the bag; that is good" << endl;
		else
			cout << "Size of the vector is not the same as the size of the bag; that is not so good" << endl;
			
		// Checks to see if the bag works with ints
		ArrayBag<int> aBagTwo;
		
		// Checks if we can add ints to the array bag
		aBagTwo.add(1);
		aBagTwo.add(2);
		aBagTwo.add(3);
		
		// Checks to see if the added ints are actually in the array bag
		if (aBagTwo.getCurrentSize() == 3)
			cout << "The correct number of ints arrays are in the array bag; that is good" << endl;
		else
			cout << "There isn't the correct number of int arrays in the array bag; that is not so good" << endl;
			
		// Checks to see if the frequency of the added int is not 0
		if (aBagTwo.getFrequencyOf(1) == 0)
			cout << "Bag can't count items correctly when they are absent; that is not so good" << endl;
		else
			cout << "Bag can count items correctly when they are absent; that is good" << endl;
	}
